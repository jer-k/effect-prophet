...
`planRollingOrigin` returns a pure, bounded rolling-origin cutoff/fold-count summary without fitting. `crossValidate` sequentially refits each planned prefix through the caller-provided `FittingBackend` Layer, predicts known future inputs with held-out targets removed and returns immutable point rows (`actual`, `predicted`, elapsed `horizonMs`, fold/cutoff). Opt in to bounded, seeded MAP value intervals with a fourth `{ mode: "intervals", uncertainty: { seed, samples?, intervalWidth? } }` argument; point prediction runs first and a typed fold error stops evaluation without a partial result. OLS has no intervals. Run the [typechecked example](examples/cross-validation.ts) with `npm run example:cross-validation:run` after building. `performanceMetrics` computes seven checked forecast metrics; `crossValidateBaseline` evaluates train-only last-observation, mean and exact-lag seasonal-naive forecasts on the same plan, and `comparePerformance` checks alignment before reporting shared-policy metrics side by side. These baselines are package extensions, not Prophet APIs. `searchModels` sequentially scores up to 32 fully specified options on identical development folds, with bounded work, deterministic ties, safe recorded failures and optional candidate-isolated interval diagnostics; see the [typechecked example](examples/model-selection.ts). Its winner is a configuration, not a fitted model or an unbiased quality estimate. `evaluateHoldout` checks a search selection receipt, fits once on development only and reports strictly later final holdout forecasts, overall metrics and train-only baselines as an experimental strict JSON-compatible report. See the [written workflow and interpretation guide](docs/usage/evaluation-and-tuning.md), [Stage H decision](docs/decisions/evaluation-and-selection.md), [modeling explanation](docs/modeling/evaluation.md) and [operational limits](docs/usage/operations.md). The former independent consumer-run requirement has been replaced with a written explanation; no independent run is claimed. EP-090's [Stage H public evaluation benchmark review](benchmark/results/evaluation/README.md) records correctness-gated absolute timings and an ineligible flat-fold discrepancy; no performance ranking is claimed. EP-085's logistic acceptance additionally requires EP-094's local fix to be merged and independently verified; no calibration claim follows from interval support.
